...
def doZakum():
    ...
    return (Inventory.GetItem(1, eye_slot).id == aquatic_letter_eye and Inventory.GetItem(1,face_slot).id == condensed_power_crystal)

def doHorntail():
    return (Inventory.GetItem(1, earring_slot).id == horntail_earrings and Inventory.GetItem(1,necklace_slot).id == horntail_necklace and (Inventory.GetItem(1,ring1_slot).id == horntail_ring or Inventory.GetItem(1,ring2_slot).id == horntail_ring or Inventory.GetItem(1,ring3_slot).id == horntail_ring or Inventory.GetItem(1,ring4_slot).id == horntail_ring))

if GameState.IsInGame():
    DoZakumDaily= not doZakum()
    DoHorntailDaily = not doHorntail()
else:
    DoZakumDaily = None
    DoHorntailDaily = None
...
if SCLib.GetVar("KillHorntailDaily") is None:
    SCLib.PersistVar("KillHorntailDaily", DoHorntailDaily)
if SCLib.GetVar("zakum_retry_count") is None:
	SCLib.PersistVar("zakum_retry_count",0)
if SCLib.GetVar("horntail_retry_count") is None:
	SCLib.PersistVar("horntail_retry_count",0)
if SCLib.GetVar("enter_cs") is None:
    if int(Terminal.GetLineEdit("LoginChar")) % 5 == 0:
...
KillHorntailDaily = SCLib.GetVar("KillHorntailDaily")
...
###CrimsonQueen Normal###
    ...
    ###Vellum Normal###
    ...
    elif KillHorntailDaily == False and  (Field.GetID() == EntranceToHorntailsCave or Field.GetID() == CaveOfLifeEntrance or Field.GetID() == CaveOfLifeEntrance1):
        LeaveHorntail()
        ...
        #StartQuest(63785, 9400428)
    elif KillHorntailDaily:
        ToggleMobDisarm(True)
        KillHorntail(2)
        if fieldID == 240050400:
            if SCLib.GetVar("horntail_retry_count") >= 4:
                SCLib.UpdateVar("KillHorntailDaily",False)
                ResetNowLockedFunction()
            else:
                SCLib.UpdateVar("horntail_retry_count",SCLib.GetVar("horntail_retry_count")+1)
                time.sleep(1)
    elif not KillVellum and not KillCrimsonQueen and not KillPierre and not KillVonBon and not KillZakumDaily and not KillHorntailDaily:
        if fieldID != mRoot:
            GotoRootAbyss()
            time.sleep(3)
        else:
