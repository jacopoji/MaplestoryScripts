...
def settings_second_job():
	...
	if not Terminal.GetCheckBox("charm_fma"):
		Terminal.SetSpinBox("charm_delay",125)
...
def settings_fourth_job():
	...
	if Terminal.GetCheckBox("MonkeySpiritsNDcheck"): #changed here to prevent monkey spirit ban
		Terminal.SetSpinBox("MonkeySpiritsNDdelay",80)
		Terminal.SetCheckBox("MonkeySpiritsNDcheck",False)
	if Terminal.GetCheckBox("bot/kanna_kami"):
		Terminal.SetCheckBox("bot/kanna_kami",False)
		Terminal.SetSpinBox("bot/kanna_kami_delay",20000)
	if not Terminal.GetCheckBox("Kami Vac"): #changed here to prevent monkey spirit ban
		Terminal.SetCheckBox("Kami Vac",True)
	if not Terminal.GetCheckBox("Skill Injection"):
		Terminal.SetCheckBox("Skill Injection",True)
...
if Character.IsOwnFamiliar(9960295) and GameState.IsInGame() and level >= 104:
...
if DoBlackGate and not SCLib.GetVar("cube_lock") and SCLib.GetVar("checked_equip") and Character.GetHP() > 0 and level >= 145 and not SCLib.GetVar("DoingMP") and not SCLib.GetVar("DoingZakum") and not SCLib.GetVar("GettingEarring"):
    toggle_rush_by_level(False)
    map = Field.GetID()
