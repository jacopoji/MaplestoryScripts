import DataType, Character, Field, Inventory, Packet, Terminal, time, GameState, functools
...
if GameState.IsInGame():
    for i in range(startSlot, endSlot+1):    
        while True:
            ...
            if not item.valid:
                print("Slot {0} is empty. Move to next".format(i))
                break
            elif stopAtMeso == True and containMesosObtained(item):
                successRoll(i)
                break
