...
#############################################################################################################################
##############################								VersionV005								#########################
##############################					Updated for Maplestory Patch V.198.1.0				#########################
#############################################################################################################################
...
##############################		When Done do you want to Set Rush By Level to keep grinding?		##############################
RushByLevel		=		False
...
MagnusHard		=		False
#-----------------------------
LucidNormal		=		False #Untested. Do not use, will be added later
#LucidHard		=		False, Will be added later.. Might take a while
#-----------------------------
CrimsonQueenNormal	=	False
...
#Holy Shit.. I made a GUI.. Check it out!
...
CrimsonQueenNormalBuffer = ["[150E0000]", "[180E0000]", "[130E0000]"]
#150E0000 180E0000 130E0000
CrimsonQueenNormalRebootBuffer = ["[FA0D0000]", "[0C0E0000]"]
#FA0D0000 0C0E0000

CrimsonQueenChaosBuffer = ["[2B110000]"]
#2B110000
CrimsonQueenChaosRebootBuffer = ["[FA0D0000]"]
#FA0D0000

VonBonNormalBuffer = ["[8E0C0000]", "[8C0C0000]"]
#8E0C0000 8C0C0000
VonBonNormalRebootBuffer =  ["[830C0000]", "[850C0000]"]
#830C0000 850C0000

VonBonChaosBuffer = ["[250F0000]"]
#250F0000
VonbonChaosRebootBuffer = ["[730C0000]"]
#730C0000

VellumNormalBuffer = ["[650E0000]", "[630E0000]"]
#650E0000 630E0000
VellumNormalRebootBuffer = ["[5A0E0000]", "[5C0E0000]"]
#5A0E0000 5C0E0000

VellumChaosBuffer = ["[DD110000]"]
#DD110000
VellumChaosRebootBuffer = ["[4A0E0000]"]
#4A0E0000

GolluxLootBuffer = ["[B6E70100]", "[B5E70100]"]
#B6E70100 B5E70100

###Packet Headers###
InteractHeader = 0x03E3
EnterBossHeader = 0x00EC
BlockBuyHeader = 0x0622
BuyItemHeader = 0x00ED
PartyHeader = 0x01A3
...
SCLib.PersistVar("CrimsonQueenChaos", CrimsonQueenChaos)
SCLib.PersistVar("PierreChaos", PierreChaos)
SCLib.PersistVar("VonBonChaos", VonBonChaos)
SCLib.PersistVar("VellumChaos", VellumChaos)
...
CrimsonQueenChaos = SCLib.GetVar("CrimsonQueenChaos")
PierreChaos = SCLib.GetVar("PierreChaos")
VonBonChaos = SCLib.GetVar("VonBonChaos")
VellumChaos = SCLib.GetVar("VellumChaos")
...
if GameState.IsInGame() and not Terminal.IsRushing():
	...
	TyrantsThroneHard = [401060100,401060101,401060102]
	...
	CrumblingClocktowerNormal = 450004250
	...
	def EnterParty():
		StartParty = Packet.COutPacket(PartyHeader)
	...
	def InteractCrimsonQueenChaos():
		print("Sending Intreact Packet")
		for x in CrimsonQueenChaosBuffer:
			Interact = Packet.COutPacket(InteractHeader)
			Interact.EncodeBuffer(x)
			Packet.SendPacket(Interact)
		for x in CrimsonQueenChaosRebootBuffer:
			InteractReboot = Packet.COutPacket(InteractHeader)
			InteractReboot.EncodeBuffer(x)
			Packet.SendPacket(InteractReboot)
	def InteractVonBonNormal():
		print("Sending Interact Packet")
		for x in VonBonNormalBuffer:
			Interact = Packet.COutPacket(InteractHeader)
			Interact.EncodeBuffer(x)
			Packet.SendPacket(Interact)
	...
	def InteractVonBonChaos():
		print("Sending Interact Packet")
		for x in VonBonChaosBuffer:
			Interact = Packet.COutPacket(InteractHeader)
			Interact.EncodeBuffer(x)
			Packet.SendPacket(Interact)
		for x in VonbonChaosRebootBuffer:
			InteractReboot = Packet.COutPacket(InteractHeader)
			InteractReboot.EncodeBuffer(x)
			Packet.SendPacket(InteractReboot)
	...
	def InteractVellumChaos():
		print("Sending Interact Packet")
		for x in VellumChaosBuffer:
			Interact = Packet.COutPacket(InteractHeader)
			Interact.EncodeBuffer(x)
			Packet.SendPacket(Interact)
		for x in VellumChaosRebootBuffer:
			InteractReboot = Packet.COutPacket(InteractHeader)
			InteractReboot.EncodeBuffer(x)
			Packet.SendPacket(InteractReboot)
	def GolluxLootPacket():
		print("Sending Gollux Loot Packet")
		for x in GolluxLootBuffer:
			Interact = Packet.COutPacket(InteractHeader)
			Interact.EncodeBuffer(x)
			Packet.SendPacket(Interact)
	...
	def CheckCompleteStepAndDeliver(questid, questnpc):
		print("Checking if {0} is complete".format(questid))
	...
	if KillZakumDaily == False and ZakumChaos == False and (fieldID == TheDoorToZakum or fieldID == EntranceToEasyZakumsAltar or fieldID == EntranceToZakumAlter):
		...
		if fieldID == TheDoorToZakum:
			print("Getting out of The Door To Zakum")
			if pos.x != -3003:
	...
	###Horntail###
		...
		if fieldID == EntranceToHorntailsCave:
			...
			if pos.x != -206:
				...
				Character.EnterPortal()
	###Magnus###
	elif KillMagnus == False and fieldID == HeliseumHeightsEntry:
		toggleHyperTeleportRock(False)
		if fieldID != TryantsCastleSittingRoomEntry:
			Terminal.Rush(TryantsCastleSittingRoomEntry)
	###Zakum###
			...
			if fieldID != EasyZakumsAltar:
				if fieldID != EntranceToEasyZakumsAltar:
					if fieldID != TheDoorToZakum:
						...
						print("Missing Zakums Stone, picking it up before entering")
						Npc.ClearSelection()
				...
				if boss.valid or boss1.valid or boss2.valid:
					...
					if HasSpawned:
						print("Zakum is dead waiting 5 sec before continue")
						toggleKami(False)
						KannaSkills(False)
						time.sleep(5)
				...
				if fieldID != EntranceToZakumAlter:
					if fieldID != TheDoorToZakum:
						...
						if pos.x != -720:
							...
							print("Missing Zakums Stone, Picking up before entering")
							Npc.ClearSelection()
							Npc.RegisterSelection("Receive an offering for Zakum.")
							time.sleep(1)
		...
		if HillaNormal:
			...
			if fieldID != HillasTowerNormal:
				...
				if boss.valid:
					...
					if HasSpawned:
						toggleKami(False)
						print("HillaNormal Defeated Will continue in 5 sec")
						time.sleep(5)
						if pos.x != -768:
			...
			if fieldID != HillasTower:
				...
				if boss.valid:
					...
					if HasSpawned:
						toggleKami(False)
						print("HillaHard Defeated Will continue in 5 sec")
						time.sleep(5)
						if pos.x != -768:
							Character.Teleport(-768, 192)
						else:
		...
		if MagnusHard:
			if fieldID not in TyrantsThroneHard:
				...
				if boss.valid or boss1.valid or boss2.valid:
					...
					toggleSI(True)
					if usingkami:
						if Terminal.GetSpinBox("KamiOffsetX") != 100:
							print("Setting Kami Vac Offset X to 100 to stay in the safe barrier")
							Terminal.SetSpinBox("KamiOffsetX", 100)
						if Terminal.GetSpinBox("KamiOffsetY") != 0:
							print("Setting Kami Vac offset Y to 0 to support all jobs")
							Terminal.SetSpinBox("KamiOffsetY", 0)
						toggleKami(True)
					...
					if HasSpawned:
						toggleKami(False)
						print("Hard Magnus is dead, Waiting 10sec before continuing")
						time.sleep(10)
						if pos.x != 595:
	...
	###Lucid###
		...
		toggleKami(False)
		if LucidNormal:
			if fieldID != CrumblingClocktowerNormal:
				if fieldID != DreamingForestNormal:
					...
					if boss.valid:
						...
						toggleNoBossMapEffect(True)
						toggleKami(True)
						print("Fighting Lucid Phase 1")
					else:
						toggleGodMode(False)
			else:
				boss = Field.FindMob(NormalLucid)
				if boss.valid:
					toggleGodMode(True)
					print("Fighting Lucid Phase 2")
				else:
					toggleGodMode(False)
	...
	###Crimson Queen Chaos###
		...
		print("Doing Crimson Queen Chaos")
			...
			if fieldID not in CastleCrimsonChaos:
				if fieldID not in SouthGardenChaos:
					...
					if not NowLockedVar:
						...
						print("Seems like you diddnt finish your last attempt and are locked. Continueing other bosses")
						SCLib.UpdateVar("CrimsonQueenChaos", False)
						ResetNowLockedFunction()
			else:
				NowLockedFunction()
				...
				if boss.valid or boss1.valid or boss2.valid or boss3.valid:
					...
					if chest.valid:
						...
						if HasSpawned:
							print("CrimsonQueen i dead and Chest Has been looted waiting 5 sec before continue")
							toggleKami(False)
							time.sleep(5)
							if pos.x != -849:
								Character.Teleport(-849, 132)
			...
			if fieldID not in AfternoonTeaTableChaos:
				if fieldID not in WestGardenChaos:
					...
					if not NowLockedVar:
						...
						print("Seems like you diddnt finish your last attempt and are locked. Continueing other bosses")
						SCLib.UpdateVar("PierreChaos", False)
						ResetNowLockedFunction()
			else:
				NowLockedFunction()
			...
			if fieldID not in TerporalCrevasseChaos:
				if fieldID not in EastGardenChaos:
					...
					if not NowLockedVar:
						...
						print("Seems like you diddnt finish your last attempt and are locked. Continueing other bosses")
						SCLib.UpdateVar("VonBonChaos", False)
						ResetNowLockedFunction()
			else:
				NowLockedFunction()
			...
			if fieldID not in AbyssalCaveChaos:
				if fieldID not in NorthGardenChaos:
					...
					if not NowLockedVar:
						...
						print("Seems like you diddnt finish your last attempt and are locked. Continueing other bosses")
						SCLib.UpdateVar("VellumChaos", False)
						ResetNowLockedFunction()
			else:
				NowLockedFunction()
	...
	###Gollux###
		...
		if fieldID != GolluxHeartLoot:
			...
			if loot.valid:
				...
				print("Gollux coin dropped, Will continue in 5 sec")
			else:
				if HasSpawned:
					time.sleep(5)
					if pos.x != 97:
					...
					if pos.x != 90:
						...
						GolluxLootPacket()
	###Deliver Daily quests if completed and go to henesys if you posess Intense Power Crystal###
	else:
		if Inventory.GetItemCount(4001886) > 0: #Intense Power Crystal
			print("You have Intense Power Crystal/s in inventory")
			if fieldID != FreeMarketEntrance:
				if fieldID != HenesysMarket:
					print("Rusing to HenesysMarket to sell Crystal/s")
					toggleHyperTeleportRock(True)
					toggleKami(False)
					Terminal.Rush(HenesysMarket)
				else:
					print("Entering Freemarket")
					if pos.x != 835:
						Character.Teleport(835, 154)
					else:
						Character.EnterPortal()
			else:
				print("Having a cheezy chat with the collector")
				Character.TalkToNpc(9001212)
				time.sleep(5)
				CloseShopWindowPacket()
		else:
			...
			CheckCompleteStepAndDeliver(26532, 9030200) #Vellum Chaos
			print("No more quests to deliver")
			if fieldID != Henesys:
				if fieldID != HenesysMarket:
					if fieldID == FreeMarketEntrance:
						if pos.x != -421:
							Character.Teleport(-421, 34)
						else:
							Character.EnterPortal()
				else:
					if pos.x != 180:
						Character.Teleport(180, 154)
					else:
						Character.EnterPortal()
			else:
				if RushByLevel:
					if not Terminal.GetCheckBox("Rush By Level"):
						print("All done Setting Rush By Level True")
						Terminal.SetCheckBox("Rush By Level", True)
###Stop rushing if in spesific map's###
else:
