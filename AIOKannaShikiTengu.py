...
def settings_second_job():
	...
	if not Terminal.GetCheckBox("charm_fma"):
		Terminal.SetSpinBox("charm_delay",105)
...
def settings_fourth_job():
	...
	if level <= 99 and GameState.IsInGame():
		...
		if Character.HasBuff(2, blackhearted) == False:
...
#		print("Sleeping for 10 seconds to farm")
#		time.sleep(10)
	if Terminal.GetComboBox("AttackKey") != 36 or not Terminal.GetCheckBox("Auto Attack") or not Terminal.GetCheckBox("charm_fma"):
		settings_third_job()
		print("Sleeping for 10 seconds to farm")
		time.sleep(10)
	if level < 120:
		...
		Terminal.SetSpinBox("FilterMeso",1000)
###### fourth job ########
if jobid == 4212 and not SCLib.GetVar("DoingMP") and not SCLib.GetVar("DoingZakum") and not SCLib.GetVar("DoingBG") and not SCLib.GetVar("BuyingExpansion"):
...
if level >= 140 and star_force and not SCLib.GetVar("DoingMP") and not SCLib.GetVar("DoingZakum") and Character.GetMeso()>= 5000000 and not SCLib.GetVar("cube_lock"):
	for equips in equip_slot_list:
		...
		if item.valid and item.id in equip_valid_list:
	...
	for accessories in accessory_slot_list:
		...
		if item.valid and item.id in accessory_list:
			#print("Starforcing item {}".format(item.id))
