'''
This is the AIOKanna version that uses shikigami charm to farm @ 105ms.
'''
...
#lucky = 2023124
#cold = 2023118
#winter_coupons = [lucky,cold]
blackhearted = 42121054
...
def settings_fourth_job():
	...
	if Terminal.GetCheckBox("Legit Vac"):
		Terminal.SetCheckBox("Legit Vac",False)
	if not Terminal.GetCheckBox("charm_fma"): #changed here to prevent monkey spirit ban
		Terminal.SetCheckBox("charm_fma",True)
	if not Terminal.GetCheckBox("Summon Kishin"):
		Terminal.SetCheckBox("Summon Kishin",True)
	if Terminal.GetCheckBox("Grenade Kami"): #changed here to prevent monkey spirit ban
		Terminal.SetCheckBox("Grenade Kami",False)
	if Terminal.GetCheckBox("MonkeySpiritsNDcheck"): #changed here to prevent monkey spirit ban
		Terminal.SetSpinBox("MonkeySpiritsNDdelay",350)
		Terminal.SetCheckBox("MonkeySpiritsNDcheck",False)
	if Terminal.GetCheckBox("bot/kanna_kami"):
		Terminal.SetCheckBox("bot/kanna_kami",False)
		Terminal.SetSpinBox("bot/kanna_kami_delay",20000)
	if Terminal.GetCheckBox("Kami Vac"): #changed here to prevent monkey spirit ban
		Terminal.SetCheckBox("Kami Vac",False)
	Terminal.SetLineEdit("SISkillID","42111011")
	Terminal.SetSpinBox("SkillInjection", 200)
	if Terminal.GetCheckBox("Skill Injection") and not accountData["cubing_done"]:
		Terminal.SetCheckBox("Skill Injection",False)
	elif not Terminal.GetCheckBox("Skill Injection") and accountData["cubing_done"]:
		Terminal.SetCheckBox("Skill Injection",True)
	...
	if level <= 99 and GameState.IsInGame():
		...
		Key.Set(0x24, 1, blackhearted)
		if Character.HasBuff(2, blackhearted) == False and SCLib.GetVar("EquipMesoDone"):
			print("Activating Blackhearted Curse")
			time.sleep(1)
			while Character.HasBuff(2, blackhearted) == False:
				print("Attempting to activate buff")
				Key.Press(0x24)
				time.sleep(1)
...
if jobid == 4200 and (level > 13 and level < 30):
	if Terminal.GetComboBox("AttackKey") != 33 or not Terminal.GetCheckBox("Auto Attack"):
		settings_first_job()
...
#		print("Sleeping for 10 seconds to farm")
#		time.sleep(10)
	if Terminal.GetComboBox("AttackKey") != 36 or not Terminal.GetCheckBox("Auto Attack") or not Terminal.GetCheckBox("charm_fma") or Terminal.GetCheckBox("Skill Injection"):
		settings_third_job()
		print("Sleeping for 10 seconds to farm")
		time.sleep(10)
	if level < 120:
...
if jobid == 4212 and not SCLib.GetVar("DoingMP") and not SCLib.GetVar("DoingZakum") and not SCLib.GetVar("DoingBG") and not SCLib.GetVar("BuyingExpansion") and not accountData['pet_expire']:
	if not Terminal.GetCheckBox("charm_fma"):# or not Terminal.GetCheckBox("Grenade Kami"): #changed here to prevent monkey spirit ban
	...
	if level > 120 and level < 145 and not SCLib.GetVar("cube_lock"):
		...
		#		if item==lucky:
		#			print("Using lucky winter")
		#		elif item == cold:
		#			print("Using cold winter")
...
#1191103 gold emblem #C:\Users\Jacopo\Pictures\MapleStoryMerch\ready_to_sell
def print_info():
...
If you think the price is too high or you want to buy multiple accounts, \nplease send me your offer, it is negotiable.
